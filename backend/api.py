...
emotion_systems = {}
energy_systems = {}
agent_memories = {}  # ShortTermMemory instances per agent
    ...
    try:
        if data.use_llm:
            ...
            if MEMORY_AVAILABLE and data.entity_id in agent_memories:
                import random
                # Simulate outcome (in real scenario, this would come from actual trade)
                outcome = "neutral" if action_data.get('action') == "hold" else "success"
                profit = 0 if action_data.get('action') == "hold" else random.uniform(-50, 100)
                ...
                print(f"[MEMORY RECORDED] {action_data.get('action')} -> {outcome} (${profit:+.0f})")
...
async def clear_memory(entity_id: str):
    """Clear an agent's memory - start fresh"""
...
if __name__ == "__main__":
    ...
    print("    ✅ Short-term memory (last 10 trades)")
    print("="*60 + "\n")
